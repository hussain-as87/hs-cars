...
<div class="card-footer">
    <div class="d-flex post-actions">
        ...
        <a class="d-flex align-items-center text-muted mr-4" data-toggle="collapse" role="button" aria-expanded="false"
           aria-controls="icons">
            <svg xmlns="http://www.w3.org/2000/svg" width="15.5" height="15.5" fill="currentColor"
    ...
    <form wire:submit.prevent="store">
        <div class="input-group col-xs-12">
            <textarea id="description" wire:model="description" class="form-control" maxlength="500" rows="2"
                      placeholder="{{ __('create new comment') }}"></textarea>
            <span class="input-group-append">
                <button class="file-upload-browse btn btn-outline-primary" type="submit"><svg
            ...
            <div class="d-flex justify-content-between mb-2 pb-2 border-bottom" id="comment-{{ $com->id }}-{{ $key }}">
                <div class="d-flex align-items-center hover-pointer">
                    <img class="img-xs rounded-circle"
                         src="@if($com->user->profile->avatar){{ asset('storage/users/avatar/'.$com->user->profile->avatar) }}@else https://ui-avatars.com/api/?name={{ auth()->user()->name }}@endif"
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton2">
                        <button class="nobu w-100" type="button" data-toggle="modal"
                                data-target="#updatecomment_{{ $com->id }}">
                            <a class="dropdown-item d-flex align-items-center"
                               href="#comment-{{ $com->id }}-{{ $key }}"><i data-feather="edit-2"
                                                                            class="icon-sm mr-2"></i> <span
                                    class="">{{ __('edit') }}</span></a>
                        </button>
                        <button type="button" class="nobu w-100" wire:click="destroy({{$com->id}})">
                            <a class="dropdown-item d-flex align-items-center" href="#"><i data-feather="trash"
                                                                                           class="icon-sm mr-2"></i>
                                <span class="">{{ __('delete') }}</span></a>
                        </button>
                    </div>
                </div>
            </div>

            <div wire:ignore.self id="updatecomment_{{ $com->id }}" class="modal fade" tabindex="-1" role="dialog"
                            ...
                            <form action="" wire:submit.prevent="update({{ $com->id }})">
                                <div class="form-group mb-0 row">
                                    ...
                                    <textarea id="description_up" wire:model="description_up"
                                              ...
                                              maxlength="500" rows="8">{{ $com->description }}</textarea>
                                    </div>
                                </div>
                                <br>
