                ...
                <table id="dataTableExample" class="table dataTable no-footer" role="grid" aria-describedby="dataTableExample_info">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $user->name }}</td>
                            <td>{{ $user->email }}</td>
                            <td class="@if ($user->status === '0')  text-danger
                                                  @else
                                                    text-success  @endif">{{ $user->status ? __('inactive') : __('active') }} </td>
                            <td>
                            ...
                            <td>
                                <a class="btn btn-info" href="{{ route('users.show', $user->id) }}">{{ __('show') }} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-columns" viewBox="0 0 16 16">
                                        <path d="M0 2a1 1 0 0 1 1-1h14a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H1a1 1 0 0 1-1-1V2zm8.5 0v8H15V2H8.5zm0 9v3H15v-3H8.5zm-1-9H1v3h6.5V2zM1 14h6.5V6H1v8z" />
                                    </svg></a>
                                @can('user-edit')
                                <a class="btn btn-primary" href="{{ route('users.edit', $user->id) }}">{{ __('edit') }} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                ...
                                <a href="" class="btn btn-danger">
                                    <form action="{{ route('users.destroy',$user->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm({{ __('are you sure?') }})" class='nobu'>{{ __('delete') }} <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
