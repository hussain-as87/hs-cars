<div class="row" wire:polling>
    <div class="col-md-7">
        ...
        <button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#storeModel"
    ...
    <!-- delete Modal -->
        <div wire:ignore.self class="modal fade" id="deleteconfirm" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-warning close-btn"
                                data-dismiss="modal">{{ __('close') }}</button>
                        <button type="button" wire:click.prevent="destroy()" class="btn btn-outline-danger close-modal"
                                data-dismiss="modal">{{ __('yes') }}
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-body">
        ...
        <div class="form-group">
            <label for="star1">
                ...
                <span class="@if($re->rating >= 1 )text-warning  @endif"><i class="ion-ios-star"></i></span>
            </label>
            <label for="star2">
                <input hidden wire:model="rating" type="radio" id="star2" value="2" />
                <span class="@if($re->rating >= 2 ) text-warning @endif"><i class="ion-ios-star"></i>
                </span> </label>
            <label for="star3">
                <input hidden wire:model="rating" type="radio" id="star3" value="3" />
                <span class="@if($re->rating >= 3 ) text-warning @endif"><i class="ion-ios-star"></i>
                </span> </label>
            <label for="star4">
                <input hidden wire:model="rating" type="radio" id="star4" value="4" />
                <span class="@if($re->rating >= 4 )text-warning  @endif"><i class="ion-ios-star"></i>
                </span> </label>
            <label for="star5">
                <input hidden wire:model="rating" type="radio" id="star5" value="5" />
                <span class="@if($re->rating >= 5 )text-warning  @endif"><i class="ion-ios-star"></i>
                </span> </label>
        </div>
...
@endif
    ...
    <div class="col-md-7">
        <div wire:ignore.self class="modal fade" id="storeModel" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">
                            <div class="spinner-border" role="status" wire:loading>
                                <span class="sr-only">Loading...</span>
                            </div> {{ __('Create New Review') }}
                        </h5>
                        <button type="button" wire:click="reset_input()" class="close" data-dismiss="modal"
                                aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form wire:submit.prevent="store()">
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="star1">
                                    <input hidden wire:model="rating" type="radio" id="star1" value="1"/>
                                    <span class="@if($rating >= 1 ) text-warning @endif"><i
                                            class="ion-ios-star"></i></span>
                                </label>
                                <label for="star2">
                                    <input hidden wire:model="rating" type="radio" id="star2" value="2"/>
                                    <span class=" @if($rating >= 2 ) text-warning @endif"><i class="ion-ios-star"></i>
                                </span> </label>
                                <label for="star3">
                                    <input hidden wire:model="rating" type="radio" id="star3" value="3"/>
                                    <span class="@if($rating >= 3 ) text-warning @endif"><i class="ion-ios-star"></i>
                                </span> </label>
                                <label for="star4">
                                    <input hidden wire:model="rating" type="radio" id="star4" value="4"/>
                                    <span class="@if($rating >= 4 ) text-warning @endif"><i class="ion-ios-star"></i>
                                </span> </label>
                                <label for="star5">
                                    <input hidden wire:model="rating" type="radio" id="star5" value="5"/>
                                    <span class="@if($rating >= 5 ) text-warning @endif"><i class="ion-ios-star"></i>
                                </span> </label>
                            </div>

                            <input type="hidden" wire:model="car_id" value="{{ $car->id }}">
                            <div class="form-group">
                                <textarea wire:model.lazy="comment" class="form-control"
                        ...
                        <div class="modal-footer">
                            <button type="button" wire:click="reset_input()" class="btn btn-outline-warning"
                                    data-dismiss="modal">{{ __('close') }}</button>
                            <button type="submit" class="btn btn-outline-primary">{{ __('Save') }}</button>
                        </div>
                    </form>
