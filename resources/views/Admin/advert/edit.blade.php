            ...
            <div class="row">
                <div class="col-lg-12 margin-tb">
                    ...
                    <div class="pull-right">
                        <a class="btn btn-info" href="{{ route('advert.index') }}"> {{ __('Go Back') }} <i
                        ...
                        <div id="home{{$key}}" class="container tab-pane {{ $loop->index == 0 ? 'show active' : '' }}"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="title[{{$key}}]"
                                       class="form-control @error('title.'.$key) is-invalid alert-danger @enderror"
                                       id="title"
                                       value="{!! old('title.'.$key,$advert->getTranslation('title',$key)) ?? $advert->title !!}"
                                       aria-describedby="emailHelp"
                                       placeholder="@error('title.'.$key){{$message}} @enderror">
                            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-12">
                            <input type="url" name="video"
                                   class="form-control @error('video') is-invalid alert-danger @enderror" id="image"
                                   value="{{ old('video') ?? $advert->video }}">
                            @error('video')
